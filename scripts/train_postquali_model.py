"""
Train post-qualifying LambdaRank models using pace + grid features.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train post-qualifying LambdaRank models")
    ...
    historical_model.save_model(models_dir / "postquali_historical_lambdarank.json")
    current_model.save_model(models_dir / f"postquali_{CURRENT_SEASON}_lambdarank.json")

    eval_scores = current_model.predict(X_current)
    ndcg = ndcg_score([y_current.to_numpy()], [eval_scores]) if len(y_current) else np.nan

    print("Saved postquali models to:")
    print(models_dir / "postquali_historical_lambdarank.json")
    print(models_dir / f"postquali_{CURRENT_SEASON}_lambdarank.json")
    print(
        f"{CURRENT_SEASON} postquali training NDCG:",
